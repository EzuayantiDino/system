...
<body>
  <div id="divMain">
    ...
    <div id="divBottom">
      <div id="divProduct">
        <h2 align="center">Accepted Order</h2><br>
        <table id="divProductList" style="width:95vw;" class="table1">
          <?php print $order->html_table_order_accepted(); ?>
        </table>
      </div>
...
<script>
  ...
  function shipping_order(rowid) {
    if(confirm("Confirm to shipping order: " + rowid) == true) {
      loadingStart();
      var data = {};
      data['action'] = "shipping_order";
      data['rowid'] = rowid;
      $.ajax({
        ...
        success: function(response) {
          console.log(response);
          if(response.result) {
            $("#divProductList").html(response.gui_table_order_accepted);
          }
          loadingEnd();
        }        
      });
