...
<body>
  <div id="divMain">
    ...
    <div id="divBottom">
      <div id="divProduct">
        <h2 align="center">Arrived Order</h2><br>
        <table id="divProductList" style="width:95vw;" class="table1">
          <?php print $order->html_table_order_arrived(); ?>
        </table>
      </div>
...
<script>
  ...
  function delete_order(rowid) {
    if(confirm("Confirm to delete order: " + rowid) == true) {
      loadingStart();
      var data = {};
      data['action'] = "delete_order";
      data['rowid'] = rowid;
      $.ajax({
        ...
        success: function(response) {
          console.log(response);
          if(response.result) {
            $("#divProductList").html(response.gui_table_order_arrived);
          }
          loadingEnd();
        }        
      });
