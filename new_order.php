...
<body>
  <div id="divMain">
    ...
    <div id="divBottom">
      <div id="divProduct">
        <h2 align="center">New Order</h2><br>
        <table id="divProductList" style="width:95vw;" class="table1">
          <?php print $order->html_table_order_new(); ?>
        </table>
      </div>
...
<script>
  ...
  function accept_order(rowid) {
    if(confirm("Confirm to accept order: " + rowid) == true) {
      loadingStart();
      var data = {};
      ...
      $.ajax({
        ...
        success: function(response) {
          console.log(response);
          if(response.result) {
            $("#divProductList").html(response.gui_table_order_new);
          }
          loadingEnd();
        }        
      });
